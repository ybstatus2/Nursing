...
export default function Layout() {
  ...
  ...(auth.currentUser?.email && ["pbhupeshk2022@gmail.com","pbhupeshk2021@gmail.com"].includes(auth.currentUser.email) ? [
  ] : []),
  ];
    ...
    <div style={{height: '100dvh', display: 'flex', flexDirection: 'column'}} className={`${bg} ${textColor} relative`}>
      ...
      <header style={{flexShrink: 0}} className={`${headerBg} border-b px-4 py-3 flex items-center justify-between z-20`}>
        <div className="flex items-center gap-3">
          <button
            ...
            className={`w-10 h-10 ${darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-gray-100 hover:bg-gray-200'} rounded-xl flex items-center justify-center transition-all active:scale-95`}
          >
            <Menu size={20} />
          </button>
      ...
      <nav style={{position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 40, paddingBottom: 'env(safe-area-inset-bottom, 0px)'}} className="px-4 pb-2 pt-1">
        <div className={`${navBg} border-t rounded-3xl shadow-2xl px-2 py-2 flex items-center justify-around mx-auto max-w-lg`}>
          {tabs.map(tab => (
            ...
            >
              {isActive(tab.id) && (
                <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-8 h-1 bg-blue-500 rounded-full" />
              )}
      ...
      {showMenu && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div
            className={`relative w-[300px] h-full shadow-2xl overflow-y-auto animate-slideInLeft ${cardBg} border-r ${cardBorder}`}
          ...
          >
            ...
            <div className={`mx-4 p-3 rounded-xl flex items-center justify-between ${darkMode ? 'bg-gray-700/50' : 'bg-gray-100'}`}>
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full relative transition-colors ${darkMode ? 'bg-blue-600' : 'bg-gray-300'}`}
              >
            ...
            <div className="p-4 mt-auto border-t border-gray-700">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-3 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 font-medium text-sm transition-all"
              >
